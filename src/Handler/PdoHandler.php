...
class PdoHandler implements HandlerInterface
{
    ...
    public function saveToken(TokenInterface $token, ?string $ownerId): bool
    {
        ...
        $t = $this->encrypter->encrypt(json_encode($token), false);
    ...
    {
        $this->table = empty($options['table']) ? 'oauth2-tokens' : $options['table'];
    }
    ...
    protected function checkDatabase(): bool
    {
        $rows = $this->pdo->exec('
            CREATE TABLE IF NOT EXISTS `' . $this->table . '` (
                ...
                `' . self::COLUMN_EXPIRES . '` BIGINT NOT NULL,
    ...
    {
        return new Token(json_decode($this->encrypter->decrypt($data[self::COLUMN_ACCESS_TOKEN], false), true));
    }
}
